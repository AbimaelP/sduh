...
export default function Maps() {
  ...
  useEffect(() => {
    ...
    if (
      (filters.search || filters.tipoImovel || filters.dormitorios) &&
      data.length > 0
    ) {
        ...
        const enderecosAtendimento =`
            ...
            <div class="flex">
                ...
                <div class="card-info-item">
                  ...
                  ${item.enderecosAtendimento && item.enderecosAtendimento.length > 0 ? 
                    item.enderecosAtendimento.map(
  ...
  useEffect(() => {
    ...
    async function initializeApp() {
      if (!window.google)
        return console.error("Google Maps script não carregado!");
        ...
        {user && user.role === "sduh_mgr" ? (
          <Section className="p-2 flex bg-white justify-between items-center">
            <Section className="flex items-center">
              ...
              >
                <Button status="EM PLANTA" className="btn btn-white">
                  Planejamento
                </Button>
                <Button status="LANÇAMENTO" className="btn btn-white ml-2">
                  Licitação
                </Button>
                <Button status="EM CONSTRUÇÃO" className="btn btn-white ml-2">
                  Em Andamento
                </Button>
                <Button status="CONSTRUÍDO" className="btn btn-white ml-2">
                  Entregues
                </Button>
